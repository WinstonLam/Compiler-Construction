 ...
 */
  if ((FALSE) || (TRUE))
    {
      CHKexistSon (DO_BLOCK (arg_node), arg_node,
		   "mandatory son DO_BLOCK is NULL");
      if (DO_BLOCK (arg_node) != NULL)
	{
	  if (!((FALSE) || (NODE_TYPE (DO_BLOCK (arg_node)) == N_while)))
	    {
	      CHKcorrectTypeInsertError (arg_node,
					 "DO_BLOCK hasnt the right type."
    ...
    {
      CHKnotExist (DO_BLOCK (arg_node), arg_node,
		   "attribute DO_BLOCK must be NULL");
    }
 ...
 */
  if ((FALSE) || (TRUE))
	...
	{
	  if (!((FALSE) || (isStmt (WHILE_EXPR (arg_node)))))
	    {
	      CHKcorrectTypeInsertError (arg_node,
...
/*
 * Attribute check: WHILE_WHILE
 */
  if ((FALSE) || (TRUE))
    {
      CHKexistAttribute (WHILE_WHILE (arg_node), arg_node,
			 "mandatory attribute WHILE_WHILE is NULL");
    }
  else
    {
      CHKnotExist (WHILE_WHILE (arg_node), arg_node,
		   "attribute WHILE_WHILE must be NULL");
    }
